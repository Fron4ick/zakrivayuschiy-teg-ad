...
function initLikeButtons() {
    ...
    iconButtonArray.forEach((iconButton, index) => {
        ...
        // Изначально скрываем overlay и outline слои
        ...
        function handleUnhover() {
            ...
            if (isMouseDown && !flag_button && !isAnimating) {
                ...
                clearOverlay();
                clearOutline();
                // Возвращаем базовое изображение
                changeBaseImage('./svg/likeicon_empty.svg');
                return;
            }

            // Если не анимируемся и не лайкнуто, возвращаем базовое состояние
            if (!flag_button && !isAnimating && !isMouseDown) {
                clearAllTimers();
                clearOverlay();
                clearOutline();
            }
        ...
        // Обработка случая, когда мышь покидает кнопку во время mousedown (глобально)
        document.addEventListener('mouseup', (e) => {
            // Если мышь была поднята вне кнопки, но mousedown был на кнопке
            if (isMouseDown && !iconButton.contains(e.target)) {
                isMouseDown = false;
                mousedownReachedFinalRed = false;
                clearAllTimers();
                clearOverlay();
                clearOutline();
                if (!flag_button && !isAnimating) {
                    changeBaseImage('./svg/likeicon_empty.svg');
                }
            }
        });
